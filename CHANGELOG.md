...
- Homebrew formula: `app "AIModelManager.app"` → `app "AI Model Manager.app"` (nome real do .app).
- Homebrew formula: homepage URL `ziguifrido/my-ai-models` → `ziguifrido/ai-model-manager`.
- Release notes: instrução `mv AIModelManager.app` → `mv "AI Model Manager.app"`.
- Template: remove comentários de placeholder que poluíam a fórmula gerada.

## [0.1.5] - 2026-06-25

### Fixed

- Homebrew formula: fix download URL (`ziguifrido/my-ai-models` → `ziguifrido/ai-model-manager`) and `depends_on macos:` deprecation warning.

## [0.1.4] - 2026-06-25

### Fixed

- Homebrew tap push: embed GH_TOKEN in git remote URL so `git push` authenticates (fatal: could not read Username).

## [0.1.3] - 2026-06-25

### Fixed

- Release workflow: add `permissions: contents: write` to job so `GITHUB_TOKEN` can create the GitHub Release (403 error).
